...
def get_load(task_list, task_num):
    ...
    book.save('../../data/scheduling_duibi/load.xls')

    for i in range(equip_num):
        # 打开Excel文件
        data = pd.read_excel('../../data/raw/result' + str(i + 1) + '.xlsx', sheetname=0)
        load_record = pd.read_excel('../../data/scheduling_duibi/load.xls')
        time_record = pd.read_excel('../../data/scheduling_duibi/predict_time_matrix.xls')
        ...
        for img in range(task_num):
            ...
            # 将更新写到新的Excel中
            DataFrame(load_record).to_excel('../../data/scheduling_duibi/load.xls')
...
def get_time_matrix(task_list, task_num):
    ...
    # 保存Excel book.save('path/文件名称.xls')
    book.save('../../data/scheduling_duibi/time_matrix.xls')

    for i in range(equip_num):
        # 打开原始数据文件
        # 打开Excel文件
        dt = pd.read_excel('../../data/raw/result' + str(i + 1) + '.xlsx')
        df = pd.read_excel('../../data/scheduling_duibi/time_matrix.xls')
        for t in range(task_num):
            # 这里求deadline需要用tet真实值
            df['equip' + str(i + 1)][t] = dt['frame_process_time'][task_list[t]]
            DataFrame(df).to_excel('../../data/scheduling_duibi/time_matrix.xls')
        print(i)


def get_deadline(task_list, task_num, proportion):
    # 打开Excel文件
    data = pd.read_excel('../../data/scheduling_duibi/time_matrix.xls')
    for temp in range(task_num):
        ...
        # 将更新写到新的Excel中
        DataFrame(data).to_excel('../../data/scheduling_duibi/deadline.xls')
    temp += 1


def get_minload(task_list, task_num):
    data = pd.read_excel('../../data/scheduling_duibi/load.xls')
    deadline = pd.read_excel('../../data/scheduling_duibi/deadline.xls')
    ...
    for i in range(task_num):
        ...
        # 将更新写到新的Excel中
        DataFrame(data).to_excel('../../data/scheduling_duibi/min_load.xls')
...
def get_weight(task_list, task_num):
    ...
    # 保存Excel book.save('path/文件名称.xls')
    book.save('../../data/scheduling_duibi/weight.xls')

    data = pd.read_excel('../../data/scheduling_duibi/min_load.xls')
    weight = pd.read_excel('../../data/scheduling_duibi/weight.xls')
    # 求30个任务的最小的负载
    for i in range(task_num):
        ...
        # 将更新写到新的Excel中
        DataFrame(weight).to_excel('../../data/scheduling_duibi/weight.xls')
        print(i)
...
def get_weight_sorted(task_list, task_num):
    ...
    # 保存Excel book.save('path/文件名称.xls')
    book.save('../../data/scheduling_duibi/weight_sorted.xls')

    weight = []
    # 根据权重对任务进行排序
    data = pd.read_excel('../../data/scheduling_duibi/weight.xls')
    dt = pd.read_excel('../../data/scheduling_duibi/weight_sorted.xls')
    ...
    for temp in range(task_num):
        ...
        data['sort'][temp] = ind
        # 将更新写到新的Excel中
        DataFrame(data).to_excel('../../data/scheduling_duibi/weight.xls')
        print(temp)
    ...
    DataFrame(dt).to_excel('../../data/scheduling_duibi/weight_sorted.xls')
...
def get_scheduling(vm):
    ...
    # 打开文件
    data = pd.read_excel('../../data/scheduling_duibi/weight_sorted.xls')
    success = 0
    scheduled = 0
    fail = 0
    times = []
    ...
    for k in range(task_num):
        ...
        # 没有调度
            ...
            data['mem_given'][k] = 0
            # 将更新写到新的Excel中
            DataFrame(data).to_excel('../../data/scheduling_duibi/schedule/' + str(vm) + '.xlsx')
...
if __name__ == '__main__':
    ...
    for i in range(15):
        sheet1.write(i + 1, 0, i)
    # 保存Excel book.save('path/文件名称.xls')
    book.save('../../data/scheduling_duibi/scheduling_duibi.xls')

    data = pd.read_excel('../../data/scheduling_duibi/scheduling_duibi.xls')
    ...
    DataFrame(data).to_excel('../../data/scheduling_duibi/scheduling_duibi.xls')
    ...
    plt.savefig('../../data/scheduling_duibi/success.png')
    plt.show()
    ...
    plt.savefig('../../data/scheduling_duibi/time.png')
    plt.show()
    ...
    plt.savefig('../../data/scheduling_duibi/energys.png')
    plt.show()
    ...
    plt.savefig('../../data/scheduling_duibi/schedule_pro.png')
    plt.show()
