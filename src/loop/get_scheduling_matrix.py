...
def schedule_matrix(i):
    ...
    # 根据任务列表求负载
    get_load(i, task_list, task_num)
    get_time_matrix(i, task_list, task_num)
    # 从time_matrix中获取deadline,按照比例
    get_deadline(i, task_list, task_num, proportion)
    get_minload(i, task_list, task_num)
    get_weight(i, task_list, task_num)
    get_weight_sorted(i, task_list, task_num)
    ##################################################
    # 虚拟机个数
    vm = 10
    successes = []
    ...
    for i in range(15):
        sheet1.write(i + 1, 0, i)
    # 保存Excel book.save('path/文件名称.xls')
    book.save('../../data/loop/matrix/0.' + str(i + 1) + '/scheduling_matrix.xls')

    data = pd.read_excel('../../data/loop/matrix/0.' + str(i + 1) + '/scheduling_matrix.xls')
    ...
    DataFrame(data).to_excel('../../data/loop/matrix/0.' + str(i + 1) + '/scheduling_matrix.xls')
    ...
    plt.savefig('../../data/loop/matrix/0.' + str(i + 1) + '/success.png')
    plt.show()
    ...
    plt.savefig('../../data/loop/matrix/0.' + str(i + 1) + '/time.png')
    plt.show()
    ...
    plt.savefig('../../data/loop/matrix/0.' + str(i + 1) + '/energys.png')
    plt.show()
    ...
    plt.savefig('../../data/loop/matrix/0.' + str(i + 1) + '/schedule_pro.png')
    plt.show()
