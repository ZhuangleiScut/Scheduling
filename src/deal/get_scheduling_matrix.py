...
def get_load(task_list, task_num):
    ...
    book.save('../../data/scheduling_matrix/load.xls')

    for i in range(equip_num):
        # 打开Excel文件
        data = pd.read_excel('../../data/raw/result' + str(i + 1) + '.xlsx', sheetname=0)
        load_record = pd.read_excel('../../data/scheduling_matrix/load.xls')
...
def get_time_matrix(task_list, task_num):
    ...
    # 保存Excel book.save('path/文件名称.xls')
    book.save('../../data/scheduling_matrix/time_matrix.xls')

    for i in range(equip_num):
        # 打开原始数据文件
        # 打开Excel文件
        dt = pd.read_excel('../../data/raw/result' + str(i + 1) + '.xlsx')
        df = pd.read_excel('../../data/scheduling_matrix/time_matrix.xls')
        for t in range(task_num):
            # 这里求deadline需要用tet真实值
            df['equip' + str(i + 1)][t] = dt['frame_process_time'][task_list[t]]
            DataFrame(df).to_excel('../../data/scheduling_matrix/time_matrix.xls')
        print(i)


def get_deadline(task_list, task_num, proportion):
    # 打开Excel文件
    data = pd.read_excel('../../data/scheduling_matrix/time_matrix.xls')
    for temp in range(task_num):
        ...
        # 将更新写到新的Excel中
        DataFrame(data).to_excel('../../data/scheduling_matrix/deadline.xls')
    temp += 1


def get_minload(task_list, task_num):
    data = pd.read_excel('../../data/scheduling_matrix/load.xls')
    deadline = pd.read_excel('../../data/scheduling_matrix/deadline.xls')
    ...
    for i in range(task_num):
        ...
        # 将更新写到新的Excel中
        DataFrame(data).to_excel('../../data/scheduling_matrix/min_load.xls')
...
def get_weight(task_list, task_num):
    ...
    # 保存Excel book.save('path/文件名称.xls')
    book.save('../../data/scheduling_matrix/weight.xls')

    data = pd.read_excel('../../data/scheduling_matrix/min_load.xls')
    weight = pd.read_excel('../../data/scheduling_matrix/weight.xls')
    # 求30个任务的最小的负载
    for i in range(task_num):
        ...
        # 将更新写到新的Excel中
        DataFrame(weight).to_excel('../../data/scheduling_matrix/weight.xls')
        print(i)
...
def get_weight_sorted(task_list, task_num):
    ...
    # 保存Excel book.save('path/文件名称.xls')
    book.save('../../data/scheduling_matrix/weight_sorted.xls')

    weight = []
    # 根据权重对任务进行排序
    data = pd.read_excel('../../data/scheduling_matrix/weight.xls')
    dt = pd.read_excel('../../data/scheduling_matrix/weight_sorted.xls')
    ...
    for temp in range(task_num):
        ...
        data['sort'][temp] = ind
        # 将更新写到新的Excel中
        DataFrame(data).to_excel('../../data/scheduling_matrix/weight.xls')
        print(temp)
    ...
    DataFrame(dt).to_excel('../../data/scheduling_matrix/weight_sorted.xls')
...
def get_scheduling(vm):
    ...
    for k in range(task_num):
        ...
        # 没有调度
            ...
            data['mem_given'][k] = 0
            # 将更新写到新的Excel中
            DataFrame(data).to_excel('../../data/scheduling_matrix/schedule/' + str(vm) + '.xlsx')
...
if __name__ == '__main__':
    ...
    plt.savefig('../../data/scheduling_matrix/success.png')
    plt.show()
    ...
    plt.savefig('../../data/scheduling_matrix/time.png')
    plt.show()
