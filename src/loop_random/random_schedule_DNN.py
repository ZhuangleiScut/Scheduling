...
def get_task_id(group_num, task_num):
    data = pd.read_excel('../../data/random_loop/group'+str(group_num)+'/task_img_id_'+str(group_num)+'.xls')
    task_list = []
...
def get_load(group_num, pro, task_list, task_num):  # pro指的是比例
    path = '../../data/random_loop/group' + str(group_num) + '/0.' + str(pro + 1) + '/DNN'
    # 构造结果表
    ...
    for i in range(equip_num):
        ...
        time_record = pd.read_excel('../../data/random_loop/group' + str(group_num) + '/0.' + str(pro + 1) + '/DNN_matrix_0.'+str(pro+1) + '.xls')
...
def get_task_position(group_num, task, task_num):
    path = '../../data/random_loop/group' + str(group_num)
...
def get_scheduling(group_num, pro, vm):
    ...
    for k in range(task_num):
        ...
        print(' - DNN-' + 'group:' + str(group_num) + '-pro:' + str(pro))
        print(' - vm:' + str(vm))
...
# task_list
# data/predict中的预测时间
def random_schedule_DNN(group_num, task_num, pro):                                 # num是第几组实验，pro是某一组实验的第几个比例
    # 根路径
    path = '../../data/random_loop/group'+str(group_num)+'/0.'+str(pro+1)+'/DNN'
    ...
    for i in range(15):
        sheet1.write(i + 1, 0, i)
    # 保存Excel book.save('path/文件名称.xls')
    book.save(path + '/scheduling_DNN.xls')

    data = pd.read_excel(path + '/scheduling_DNN.xls')
    ...
    DataFrame(data).to_excel(path + '/scheduling_DNN.xls')
    ...
    plt.savefig(path + '/schedule_pro.png')
    plt.show()
