...
def get_load(group_num, pro, task_list, task_num):  # pro指的是比例
    path = '../../data/random_loop/group' + str(group_num) + '/0.' + str(pro + 1) + '/matrix'
    # 构造结果表
...
def get_scheduling(group_num, pro, vm):
    ...
    for k in range(task_num):
        ...
        # 没有调度
            ...
            data['mem_given'][k] = 0
            # 将更新写到新的Excel中
            DataFrame(data).to_excel(path + '/schedule/matrix_' + str(vm) + '.xlsx')
            continue
...
# task_list
# data/predict中的预测时间
def random_schedule_matrix(group_num, task_num, pro):  # num是第几组实验，pro是某一组实验的第几个比例
    # 根路径
    path = '../../data/random_loop/group' + str(group_num) + '/0.' + str(pro + 1) + '/matrix'
